...
(() => {
  ...
  function renderOverview(data) {
    ...
    // Achievement Spam Days (replaces Highest Point — WotLK achievements are mostly 10pts)
    const spamDays = Array.isArray(data.achievement_spam_days) ? data.achievement_spam_days : [];
    if (spamDays.length > 0) {
      ...
      spamDays.forEach(day => {
        const card = document.createElement('div');
        card.className = 'spam-day-card';

        const dateEl = document.createElement('div');
        dateEl.className = 'spam-day-date';
        dateEl.textContent = day.achievement_date;
        card.appendChild(dateEl);

        const stats = document.createElement('div');
        stats.className = 'spam-day-stats';

        const countEl = document.createElement('div');
        countEl.className = 'spam-day-count';
        countEl.textContent = `${day.achievement_count} achievements`;
        stats.appendChild(countEl);

        const ptsEl = document.createElement('div');
        ptsEl.className = 'spam-day-points';
        ptsEl.textContent = `${day.points_earned} points`;
        stats.appendChild(ptsEl);
  ...
  function renderCollections(data) {
    ...
    if (data.mounts && data.mounts.length > 0) {
      ...
      data.mounts.forEach(mount => {
        ...
        if (window.WDTooltip && mount.spell_id) {
          WDTooltip.attach(card, { item_id: mount.spell_id, item_type: 'spell', name: mount.name }, null);
        }
    ...
    if (data.pets && data.pets.length > 0) {
      ...
      data.pets.forEach(pet => {
        ...
        if (window.WDTooltip && pet.spell_id) {
          WDTooltip.attach(card, { item_id: pet.spell_id, item_type: 'spell', name: pet.name }, null);
        }
  ...
  function renderTimeline(data) {
    ...
    timelineSection.className = 'ach-timeline-section';
    
    const header = document.createElement('div');
    header.className = 'ach-timeline-header';
    header.innerHTML = `
      <h3>📅 Achievement Timeline</h3>
      <input type="text" id="timelineSearch" class="ach-timeline-search" placeholder="Search achievements...">
    `;
    timelineSection.appendChild(header);

    if (data.achievement_timeline && data.achievement_timeline.length > 0) {
      const timelineList = document.createElement('div');
      timelineList.className = 'ach-timeline-list';
  ...
  function filterAndRenderTimeline(allData, container) {
    ...
    filtered.forEach(ach => {
      const item = document.createElement('div');
      item.className = 'ach-timeline-item';

      const dateDiv = document.createElement('div');
      dateDiv.className = 'ach-timeline-date';
      dateDiv.textContent = formatDate(ach.earned_date);
      item.appendChild(dateDiv);

      const achDiv = document.createElement('div');
      achDiv.className = 'ach-timeline-achievement';

      const nameDiv = document.createElement('div');
      nameDiv.className = 'ach-timeline-name';
      nameDiv.textContent = ach.name || '';
      achDiv.appendChild(nameDiv);

      if (ach.description) {
        const descDiv = document.createElement('div');
        descDiv.className = 'ach-timeline-desc';
        descDiv.textContent = ach.description;
        achDiv.appendChild(descDiv);
      }

      const ptsDiv = document.createElement('div');
      ptsDiv.className = 'ach-timeline-points';
      ptsDiv.textContent = `${ach.points} points`;
      achDiv.appendChild(ptsDiv);

      item.appendChild(achDiv);
