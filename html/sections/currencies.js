...
/* WhoDASH Currencies â€” Complete Enhanced Version with ALL Analytics */
(() => {
  // ===== Helpers =====
  ...
  function renderNetWorthChart(timeseries) {
    ...
    title.innerHTML = '📈 Net Worth (All Time)';
    ...
    canvas.width = 1400;  // Increased from 800 for full width
    canvas.height = 300;  // Increased from 200 for better visibility
    ...
    ctx.font = '14px system-ui';  // Slightly larger font
    ctx.textAlign = 'right';
    for (let i = 0; i <= 5; i++) {
      const val = Math.round(minVal + (range * i / 5));
      const gold = Math.floor(val / 10000);
      const y = padding + h - (i / 5) * h;
      ctx.fillText(`${gold.toLocaleString()}g`, padding - 10, y + 4);
    }
    ...
    if (timeseries.length > 0) {
      // First date
      const firstDate = new Date(timeseries[0].ts * 1000).toLocaleDateString();
      ctx.fillText(firstDate, padding, canvas.height - 10);
      
      // Middle date
      const midIdx = Math.floor(timeseries.length / 2);
      const midDate = new Date(timeseries[midIdx].ts * 1000).toLocaleDateString();
      ctx.fillText(midDate, padding + w / 2, canvas.height - 10);
      
      // Last date
      const lastDate = new Date(timeseries[timeseries.length - 1].ts * 1000).toLocaleDateString();
      ctx.fillText(lastDate, padding + w, canvas.height - 10);
    }
  ...
  function renderGoldPerHour(gphData) {
    ...
    const sorted = gphData
      ...
      .slice(0, 20);  // Increased from 10 to 20 for full-width display
    ...
    avgDiv.innerHTML = `Average: <strong>${formatGold(Math.round(avg))}/hr</strong> (Top ${sorted.length} sessions)`;
  ...
  function renderRecentLoot(loot) {
    ...
    validLoot.forEach(item => {
      ...
      if (window.WDTooltip && item.link) {
        WDTooltip.attach(row, { link: item.link, name: item.displayName, icon: item.icon }, null);
      }
  ...
  function renderAuctionPerformance(auctions) {
    ...
    card.innerHTML = `
      <h2 class="finance-title">🛒 Auction House Performance</h2>
      <div class="auction-stats-grid">
        <div class="auction-stat">
          <div class="stat-label">Total Auctions (All Time))</div>
          <div class="stat-value">${stats.total_auctions_historical || 0}</div>
        </div>
        <div class="auction-stat">
          <div class="stat-label">Total Value (All Time)</div>
          <div class="stat-value">${formatGold(stats.total_value_historical || 0)}</div>
        </div>
        <div class="auction-stat">
  ...
  function renderDepositBurnVsMargin(data) {
    ...
    card.innerHTML = `
      <h3>💸 Deposit Burn vs. Revenue</h3>
      <p class="muted" style="font-size:0.85rem;margin:0 0 12px;">
        Revenue = total gold received from sales. Deposit burn = estimated lost deposits from expired listings.
        Sorted by total listings (most active items first).
      </p>
      <div class="deposit-table-container"></div>
    `;

    if (!data || data.length === 0) {
      card.querySelector('.deposit-table-container').innerHTML = '<p class="muted">No data available</p>';
      return card;
    }
    ...
    const tbody = document.createElement('tbody');
    data.forEach(item => {
      const sellRate = item.sell_rate ?? (item.total_count > 0 ? Math.round((item.sold_count / item.total_count) * 100) : 0);
      const rateClass = sellRate >= 80 ? 'excellent' : sellRate >= 50 ? 'good' : 'poor';
      const burnClass = item.deposit_burn > 0 ? 'style="color:#ef4444"' : '';
      ...
      tr.innerHTML = `
        ...
        <td class="success-rate ${rateClass}">${sellRate}%</td>
        <td>${item.expire_count > 0 ? `<span style="color:#f59e0b">${item.expire_count}</span>` : '<span class="muted">0</span>'}</td>
        <td ${burnClass}>${item.deposit_burn > 0 ? formatGold(item.deposit_burn) : '<span class="muted">—</span>'}</td>
        <td style="color:#10b981">${item.total_revenue > 0 ? formatGold(item.total_revenue) : '<span class="muted">—</span>'}</td>
      `;
      tbody.appendChild(tr);
    });

    table.appendChild(tbody);
    card.querySelector('.deposit-table-container').appendChild(table);
    return card;
  }

  // Filter out placeholder names generated when no item name is available
  function isKnownName(name) {
    if (!name) return false;
    if (/^Item #\d+$/i.test(name)) return false;
    if (/^unknown$/i.test(name.trim())) return false;
    return true;
  }
  ...
  function renderPriceHistory(data) {
    ...
    if (!data || data.length === 0) {
      card.querySelector('.price-history-charts').innerHTML = `
        <div class="empty-state">
          <p class="muted">No market price data available.</p>
          <p class="muted" style="font-size: 0.9rem; margin-top: 8px;">
            This feature requires market snapshot data from the auction house. 
            Make sure you're uploading data that includes competitor pricing information.
          </p>
        </div>
      `;
      return card;
    ...
    data.filter(item => isKnownName(item.item_name)).slice(0, 3).forEach(item => {
  ...
  function renderUndercutLadder(data) {
    ...
    if (!data || data.length === 0) {
      card.querySelector('.undercut-list').innerHTML = `
        <div class="empty-state">
          <p class="muted">No undercut opportunities available.</p>
          <p class="muted" style="font-size: 0.9rem; margin-top: 8px;">
            This requires both active auctions and market snapshot data to compare your prices against competitors.
          </p>
        </div>
      `;
      return card;
  ...
  function renderTopItemsPerformance(data) {
    ...
    data.filter(item => isKnownName(item.name)).forEach(item => {
      const row = document.createElement('tr');
  ...
  function renderCompetitorAnalysis(data) {
    ...
    card.innerHTML = `
      ...
      <div class="competitor-grid"></div>
    `;
    
    if (!data || data.length === 0) {
      card.querySelector('.competitor-grid').innerHTML = '<p class="muted">No competitor data. Upload market snapshots to track competition!</p>';
      return card;
    }
    
    const gridDiv = card.querySelector('.competitor-grid');
    
    // Show up to 20 competitors in a grid layout
    data.filter(comp => isKnownName(comp.seller)).slice(0, 20).forEach((comp, index) => {
      const compDiv = document.createElement('div');
      compDiv.className = 'competitor-card';
      
      // Calculate some stats
      const avgPrice = comp.items.length > 0 
        ? Math.round(comp.items.reduce((sum, item) => sum + item.avg_price, 0) / comp.items.length)
        : 0;
      
      const lowestOverall = comp.items.length > 0
        ? Math.min(...comp.items.map(item => item.lowest_price))
        : 0;
      
      compDiv.innerHTML = `
        <div class="competitor-rank-badge">#${index + 1}</div>
        <div class="competitor-name-lg">${comp.seller}</div>
        <div class="competitor-stats-row">
          <div class="competitor-stat">
            <div class="stat-value">${comp.total_items}</div>
            <div class="stat-label">Items</div>
          </div>
          <div class="competitor-stat">
            <div class="stat-value">${comp.total_snapshots}</div>
            <div class="stat-label">Listings</div>
          </div>
          <div class="competitor-stat">
            <div class="stat-value">${formatGoldPlain(avgPrice)}</div>
            <div class="stat-label">Avg Price</div>
          </div>
        </div>
        <div class="competitor-items-preview">
          ${comp.items.filter(item => isKnownName(item.item_name)).slice(0, 5).map(item => `
            <div class="competitor-item-mini">
              <span class="item-name-mini" title="${item.item_name}">${item.item_name}</span>
              <span class="item-price-mini">${formatGoldPlain(item.lowest_price)}</span>
            </div>
          `).join('')}
          ${comp.items.length > 5 ? `<div class="competitor-more-mini">+${comp.items.length - 5} more</div>` : ''}
        </div>
      `;
  ...
  function initTabs() {
    ...
    tabContainer.innerHTML = `
      ...
      <button class="currency-tab" data-tab="auctions">🛒 Auction House</button>
  ...
  function renderEconomyTab(root, data) {
    ...
    // Filter out zero values from timeseries for cleaner chart
    const cleanedTimeseries = (data.timeseries || []).filter(d => d.value > 0);

    // Net Worth Chart - Full Width
    const networthSection = document.createElement('div');
    networthSection.className = 'dashboard-section';
    networthSection.appendChild(renderNetWorthChart(cleanedTimeseries));
    root.appendChild(networthSection);

    // Gold Per Hour - Full Width
    const gphSection = document.createElement('div');
    gphSection.className = 'dashboard-section';
    gphSection.appendChild(renderGoldPerHour(data.gold_per_hour || []));
    root.appendChild(gphSection);

    // Milestones and Recent Loot side by side at bottom
  ...
  function renderAuctionTab(root, data) {
    ...
    // Row 1: Core Performance (unchanged)
    ...
    // Row 2: Top Items Performance & Deposit Burn (moved Top Items Performance here)
    const row2 = document.createElement('div');
    row2.className = 'analytics-row';
    row2.appendChild(renderTopItemsPerformance(data.auctions?.top_items_performance || []));
    row2.appendChild(renderDepositBurnVsMargin(data.auctions?.deposit_vs_margin || []));
    analyticsGrid.appendChild(row2);
    
    // Row 3: Repost Tracker & Profit Calculator (moved Repost Tracker here)
    const row3 = document.createElement('div');
    row3.className = 'analytics-row';
    row3.appendChild(renderRepostTracker(data.auctions?.repost_tracker || []));
    row3.appendChild(renderProfitCalculator());
    analyticsGrid.appendChild(row3);
    
    // Row 4: Competitor Analysis - Full Width
    const row4 = document.createElement('div');
    row4.className = 'analytics-row full-width';
    row4.appendChild(renderCompetitorAnalysis(data.auctions?.competitor_analysis || []));
    analyticsGrid.appendChild(row4);
    
    // Row 5: Price History & Undercut Opportunities at bottom
    const row5 = document.createElement('div');
    row5.className = 'analytics-row';
    row5.appendChild(renderPriceHistory(data.auctions?.price_history || []));
    row5.appendChild(renderUndercutLadder(data.auctions?.undercut_opportunities || []));
