...
/* WhoDASH Progression – Raid & Boss Kill Tracker (ENHANCED) */
(() => {
  ...
  function renderOverview(data) {
    ...
    // Enhanced Stats Grid - First Row: Basic Stats
    ...
    // Second Row - Combat Performance Stats (if available)
    if (data.boss_combat_stats && data.boss_combat_stats.bosses_with_combat_data > 0) {
      const combatGrid = document.createElement('div');
      combatGrid.className = 'prog-stats-grid prog-combat-stats';
      combatGrid.innerHTML = `
        <div class="prog-stat-card prog-combat-card">
          <div class="stat-icon">💥</div>
          <div class="stat-value">${data.boss_combat_stats.avg_boss_dps.toLocaleString()}</div>
          <div class="stat-label">Avg Boss DPS</div>
          <div class="stat-detail">Max: ${data.boss_combat_stats.max_boss_dps.toLocaleString()} DPS</div>
        </div>
        
        <div class="prog-stat-card prog-combat-card">
          <div class="stat-icon">⏱️</div>
          <div class="stat-value">${formatDuration(data.boss_combat_stats.avg_boss_fight_duration)}</div>
          <div class="stat-label">Avg Fight Duration</div>
          <div class="stat-detail">Based on ${data.boss_combat_stats.bosses_with_combat_data} bosses</div>
        </div>
        
        <div class="prog-stat-card prog-combat-card ${data.best_boss_performance ? '' : 'muted'}">
          <div class="stat-icon">🏆</div>
          <div class="stat-value">${data.best_boss_performance ? data.best_boss_performance.dps.toLocaleString() : 'N/A'}</div>
          <div class="stat-label">Best Boss DPS</div>
          <div class="stat-detail">${data.best_boss_performance ? data.best_boss_performance.boss_name : 'No data'}</div>
        </div>
      `;
      container.appendChild(combatGrid);
    }

    // Raid Progression Section
    const raidProgressionData = Array.isArray(data.raid_progression) ? data.raid_progression : [];
    if (raidProgressionData.length > 0) {
      ...
      // Group by instance
      const byInstance = {};
      raidProgressionData.forEach(prog => {
    ...
    const diffBreakdownData = Array.isArray(data.difficulty_breakdown) ? data.difficulty_breakdown : [];
    if (diffBreakdownData.length > 0) {
      ...
      // Render pie chart
      setTimeout(() => renderDifficultyPieChart(canvas, legend, diffBreakdownData), 0);
    }
  ...
  // ===== ENHANCED Boss Kills Tab with Tooltips =====
  function renderBossKills(data) {
    ...
    const allBossKillsData = Array.isArray(data.all_boss_kills) ? data.all_boss_kills : [];
    allBossKillsData.forEach(k => {
    ...
    setTimeout(() => {
      ...
      if (searchInput) {
        searchInput.addEventListener('input', () => filterAndRenderBossKills(allBossKillsData, container));
      }
      if (diffFilter) {
        diffFilter.addEventListener('change', () => filterAndRenderBossKills(allBossKillsData, container));
      }
      if (clearBtn) {
        clearBtn.addEventListener('click', () => {
          if (searchInput) searchInput.value = '';
          if (diffFilter) diffFilter.value = '';
          filterAndRenderBossKills(allBossKillsData, container);
        });
      }
    }, 0);
    ...
    tableWrapper.className = 'boss-kills-table-wrapper';
  ...
  function filterAndRenderBossKills(allData, container) {
    ...
    tbody.innerHTML = filtered.map(kill => {
      ...
      const hasCombatData = kill.dps && kill.dps > 0;
      
      return `
        <tr class="boss-kill-row ${hasCombatData ? 'has-combat-data' : ''}" data-kill='${JSON.stringify(kill)}'>
          ...
          <td class="combat-stats-cell">
            ${hasCombatData ? `
              <span class="combat-indicator" title="Combat data available">
                💥 ${Math.round(kill.dps).toLocaleString()} DPS
              </span>
  ...
  function setupTooltips(container) {
    const rows = qa('.boss-kill-row.has-combat-data', container);
  ...
  function showTooltip(e, row) {
    const killData = JSON.parse(row.dataset.kill);
    
    // Remove any existing tooltips
    hideTooltip();
    
    const tooltip = document.createElement('div');
    tooltip.className = 'boss-kill-tooltip';
    tooltip.id = 'bossKillTooltip';
    
    tooltip.innerHTML = `
      <div class="tooltip-header">
        <div class="tooltip-boss-name">${killData.boss_name}</div>
        <div class="tooltip-instance">${killData.instance} - ${killData.difficulty_name}</div>
      </div>
      <div class="tooltip-divider"></div>
      <div class="tooltip-stats">
        <div class="tooltip-stat-row">
          <span class="tooltip-label">💥 DPS:</span>
          <span class="tooltip-value">${Math.round(killData.dps).toLocaleString()}</span>
        </div>
        ${killData.hps ? `
          <div class="tooltip-stat-row">
            <span class="tooltip-label">💚 HPS:</span>
            <span class="tooltip-value">${Math.round(killData.hps).toLocaleString()}</span>
          </div>
        ` : ''}
        ${killData.dtps ? `
          <div class="tooltip-stat-row">
            <span class="tooltip-label">🛡️ DTPS:</span>
            <span class="tooltip-value">${Math.round(killData.dtps).toLocaleString()}</span>
          </div>
        ` : ''}
        <div class="tooltip-stat-row">
          <span class="tooltip-label">⏱️ Duration:</span>
          <span class="tooltip-value">${formatDuration(killData.duration)}</span>
        </div>
        ${killData.total_damage ? `
          <div class="tooltip-stat-row">
            <span class="tooltip-label">🎯 Total Damage:</span>
            <span class="tooltip-value">${formatNumber(killData.total_damage)}</span>
          </div>
        ` : ''}
        ${killData.total_healing ? `
          <div class="tooltip-stat-row">
            <span class="tooltip-label">💊 Total Healing:</span>
            <span class="tooltip-value">${formatNumber(killData.total_healing)}</span>
          </div>
        ` : ''}
        ${killData.total_damage_taken ? `
          <div class="tooltip-stat-row">
            <span class="tooltip-label">⚔️ Damage Taken:</span>
            <span class="tooltip-value">${formatNumber(killData.total_damage_taken)}</span>
          </div>
        ` : ''}
        ${killData.overheal_pct ? `
          <div class="tooltip-stat-row">
            <span class="tooltip-label">🌊 Overheal:</span>
            <span class="tooltip-value">${Math.round(killData.overheal_pct)}%</span>
          </div>
        ` : ''}
        ${killData.target_level ? `
          <div class="tooltip-stat-row">
            <span class="tooltip-label">📊 Boss Level:</span>
            <span class="tooltip-value">${killData.target_level}</span>
          </div>
        ` : ''}
      </div>
      <div class="tooltip-footer">
        ${formatDateTime(killData.ts)}
      </div>
    `;
  ...
  function hideTooltip() {
    const tooltip = document.getElementById('bossKillTooltip');
    if (tooltip) {
      tooltip.remove();
    }
  }
  ...
  function renderLockouts(data) {
    ...
    if (data.active_lockouts && Array.isArray(data.active_lockouts) && data.active_lockouts.length > 0) {
    ...
    // Lockout History
    if (data.lockout_history && Array.isArray(data.lockout_history) && data.lockout_history.length > 0) {
