...
try {
    ...
    if ($hasQuestLog) {
        ...
        if ($latestTs) {
            ...
            $stmt = $pdo->prepare('
                ...
                ORDER BY quest_id
            ');
            $stmt->execute([$character_id, $latestTs]);
    ...
    $stmt = $pdo->prepare('
        SELECT quest_title, money, xp, reward_chosen_name, reward_chosen_quality, reward_chosen_link
        FROM quest_rewards
        WHERE character_id = ?
    ...
    // Epic reward quests
    $stmt = $pdo->prepare('
        SELECT quest_title, zone, reward_chosen_name, reward_chosen_quality, reward_chosen_link, ts
        FROM quest_rewards
        WHERE character_id = ?
    ...
    $stmt = $pdo->prepare('
        SELECT 
            reward_chosen_name,
            reward_chosen_quality,
            MAX(reward_chosen_link) as reward_chosen_link,
            COUNT(*) as times_chosen
        FROM quest_rewards
        WHERE character_id = ?
            AND reward_chosen_name IS NOT NULL
