...
(() => {
    ...
    if (data.most_rewarding_quest) {
      ...
      rewardCard.innerHTML = `
        ...
        <div class="rewarding-quest-details">
          ...
          ${quest.reward_chosen_name ? `<div class="mrq-reward-line" style="color: ${getQualityColor(quest.reward_chosen_quality)}">🎁 ${quest.reward_chosen_name}</div>` : ''}
        </div>
      `;
      // Attach tooltip to the reward line if we have a link
      if (window.WDTooltip && quest.reward_chosen_link && quest.reward_chosen_name) {
        const rewardLine = rewardCard.querySelector('.mrq-reward-line');
        if (rewardLine) {
          WDTooltip.attach(rewardLine, { link: quest.reward_chosen_link, name: quest.reward_chosen_name }, null);
        }
      }
  ...
  function filterAndRenderQuestLog(allData, container) {
    ...
    filtered.forEach(quest => {
      ...
      // Build all static cells via innerHTML, leaving reward cell empty for DOM attachment
      tr.innerHTML = `
        <td><strong>${quest.quest_title || 'Unknown'}</strong></td>
        <td>${quest.zone || '—'}</td>
        <td>${quest.quest_level || '?'}</td>
        <td class="quest-reward-cell"></td>
        <td>${quest.xp ? quest.xp.toLocaleString() : '0'}</td>
        <td>${quest.money ? formatGold(quest.money) : '0c'}</td>
        <td class="muted">${formatDate(quest.ts)}</td>
      `;
      // Populate reward cell and optionally attach tooltip
      const rewardCell = tr.querySelector('.quest-reward-cell');
      rewardCell.textContent = quest.reward_chosen_name || 'None';
      rewardCell.style.color = rewardColor;
      if (window.WDTooltip && quest.reward_chosen_link && quest.reward_chosen_name) {
        WDTooltip.attach(rewardCell, { link: quest.reward_chosen_link, name: quest.reward_chosen_name }, null);
      }
  ...
  function renderRewards(data) {
    ...
    if (data.top_chosen_rewards && data.top_chosen_rewards.length > 0) {
      ...
      data.top_chosen_rewards.forEach(reward => {
        ...
        `;
        if (window.WDTooltip && reward.reward_chosen_link) {
          WDTooltip.attach(card, { link: reward.reward_chosen_link, name: reward.reward_chosen_name }, null);
        }
