...
(() => {
  ...
  const LOG_STEPS = [2000, 4000, 6000, 8000, 10000]; // incremental load steps
  ...
  function renderLog(data) {
    ...
    // Load-more bar
    container.appendChild(buildLoadMoreBar(data, container));

    // Store data for filtering
    container.dataset.zoneData   = JSON.stringify(data.recent_zones);
    container.dataset.currentLimit = String(data.recent_zones_limit || 2000);
    container.dataset.totalRows    = String(data.total_zone_rows   || 0);
    ...
    setTimeout(() => {
      ...
      // Server-side search: fires when user types, debounced
      ...
      const handleSearchInput = () => {
        ...
        if (term.length === 0) {
          // Cleared — revert to local chunk filtering
          container.dataset.searchActive = 'false';
          applyLocalFilters();
          return;
        }
        ...
        searchTimer = setTimeout(() => serverSearch(term, container), SEARCH_DEBOUNCE_MS);
      };

      const applyDateFilters = () => {
        // Date filters always work locally against whatever data is loaded
        const allData = JSON.parse(container.dataset.zoneData);
        filterAndRenderLogTable(allData, container);
      };

      if (searchInput) searchInput.addEventListener('input', handleSearchInput);
      if (dateFrom)    dateFrom.addEventListener('change',  applyDateFilters);
      if (dateTo)      dateTo.addEventListener('change',    applyDateFilters);
      if (clearBtn) {
  ...
  function buildLoadMoreBar(data, container) {
    const totalRows   = data.total_zone_rows   || 0;
    const currentLimit = data.recent_zones_limit || 2000;

    const bar = document.createElement('div');
    bar.className   = 'log-load-more-bar';
    bar.id          = 'logLoadMoreBar';

    // Nothing more to load
    if (totalRows <= currentLimit || currentLimit === 0) {
      return bar; // empty, invisible
    }

    const label = document.createElement('span');
    label.className   = 'log-load-label';
    label.textContent = `Loaded ${currentLimit.toLocaleString()} of ${totalRows.toLocaleString()} total visits — load more:`;
    bar.appendChild(label);

    const btnGroup = document.createElement('div');
    btnGroup.className = 'log-load-btns';

    // Step buttons
    LOG_STEPS.forEach(step => {
      if (step <= currentLimit) return; // already loaded this many
      if (step > totalRows && step !== LOG_STEPS[LOG_STEPS.indexOf(step)]) return;

      const btn = document.createElement('button');
      btn.className   = 'log-load-btn';
      btn.textContent = step.toLocaleString();
      btn.title       = `Load ${step.toLocaleString()} most recent visits`;
      btn.addEventListener('click', () => reloadLogData(step, container));
      btnGroup.appendChild(btn);
    });

    // Yolo button — only if there's more beyond 10000
    if (totalRows > (currentLimit)) {
      const yolo = document.createElement('button');
      yolo.className   = 'log-load-btn log-load-btn--yolo';
      yolo.textContent = '☠ Load it all, yolo.';
      yolo.title       = `Load all ${totalRows.toLocaleString()} visits — this may take a very long time`;
      yolo.addEventListener('click', () => {
        if (confirm(`You're about to load all ${totalRows.toLocaleString()} zone visits.\n\nThis could take a very long time depending on your history. Are you sure?`)) {
          reloadLogData(0, container);
        }
      });
      btnGroup.appendChild(yolo);
    }
  ...
  async function reloadLogData(limit, container) {
    const section     = q('#tab-travel-log');
    const characterId = section ? section.dataset.characterId : null;
    if (!characterId) return;

    // Show loading state in bar
    const bar = q('#logLoadMoreBar', container);
    if (bar) {
      bar.innerHTML = `<span class="log-load-label log-load-loading">⏳ Loading ${limit > 0 ? limit.toLocaleString() : 'all'} visits…</span>`;
    }

    try {
      const url = `/sections/travel-log-data.php?character_id=${characterId}&limit=${limit}&zone_log_only=1`;
      const res = await fetch(url, { credentials: 'include' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();

      const zones = data.recent_zones || [];

      // Update stored data and re-render table
      container.dataset.zoneData    = JSON.stringify(zones);
      container.dataset.currentLimit = String(limit);
      container.dataset.totalRows    = String(data.total_zone_rows || 0);

      filterAndRenderLogTable(zones, container);

      // Rebuild load-more bar
      if (bar) {
        const newBar = buildLoadMoreBar(
          { recent_zones: zones, recent_zones_limit: limit, total_zone_rows: data.total_zone_rows },
          container
        );
        newBar.id = 'logLoadMoreBar';
        bar.replaceWith(newBar);
      }

    } catch (err) {
      if (bar) bar.innerHTML = `<span class="log-load-label" style="color:#f44">Error loading data: ${err.message}</span>`;
    }
  }

  async function serverSearch(term, container) {
    const section     = q('#tab-travel-log');
    const characterId = section ? section.dataset.characterId : null;
    if (!characterId) return;
    ...
    if (paginationInfo) {
      paginationInfo.textContent = `🔍 Searching full history for "${term}"…`;
    }

    try {
      const url = `/sections/travel-log-data.php?character_id=${characterId}&zone_log_only=1&search=${encodeURIComponent(term)}`;
      const res = await fetch(url, { credentials: 'include' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      ...
      // Store search results as the active data set
      container.dataset.zoneData     = JSON.stringify(zones);
      container.dataset.searchActive = 'true';

      // Render with date filters still applied if set
      filterAndRenderLogTable(zones, container, { searchOverride: term, totalRows: data.total_zone_rows });

    } catch (err) {
      if (paginationInfo) {
        paginationInfo.textContent = `Error searching: ${err.message}`;
      }
    }
  }


  function filterAndRenderLogTable(allData, container, opts = {}) {
    const searchInput    = q('#zoneSearchInput', container);
    const dateFrom       = q('#dateFromInput',   container);
    const dateTo         = q('#dateToInput',     container);
    ...
    // When a server search is active, skip the local zone text filter
    // (data already pre-filtered by server) — only apply date filters locally
    const isServerSearch = container.dataset.searchActive === 'true';
    const searchTerm     = (!isServerSearch && searchInput) ? searchInput.value.toLowerCase() : '';
    const fromDate       = dateFrom && dateFrom.value ? new Date(dateFrom.value).getTime() / 1000 : null;
    const toDate         = dateTo   && dateTo.value   ? new Date(dateTo.value).getTime()   / 1000 + 86400 : null;
    ...
    if (paginationInfo) {
      ...
      const totalAll    = opts.totalRows != null
        ? opts.totalRows
        : parseInt(container.dataset.totalRows || '0', 10);

      if (isServerSearch || opts.searchOverride) {
        const term = opts.searchOverride || (searchInput ? searchInput.value.trim() : '');
        paginationInfo.innerHTML =
          `🔍 <strong>${filtered.length.toLocaleString()}</strong> results for "<em>${term}</em>" from full history` +
          (fromDate || toDate ? ` (date filtered)` : '');
      } else {
        const capNote = totalAll > totalLoaded ? ` of ${totalAll.toLocaleString()} total` : '';
        paginationInfo.textContent =
          `Showing ${filtered.length.toLocaleString()} of ${totalLoaded.toLocaleString()} loaded${capNote}`;
      }
    }
  }
